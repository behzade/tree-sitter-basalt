...
const PREC = {
  assign: -1,
  with: 1, lambda: 2, or: 3, and: 4, equality: 5, comparison: 6,
  term: 7, factor: 8, unary: 9, return: 10,
  call: 11, path: 12, member: 13, pattern: 14, struct_literal: 15,
};
...
module.exports = grammar({
  ...
  rules: {
    ...
    _declaration: $ => choice(
      $.import_declaration, $.type_definition, $.implementation,
    ...
    type_definition: $ => seq(
      optional('pub'), field('name', $.identifier), ':',
      field('body', choice(
        $.struct_definition, $.interface_definition, $.enum_definition, $.effect_definition
      ))
    ),
    ...
    implementation: $ => seq(
      field('type', $.identifier), ':', 'impl', field('interface', $.identifier),
    ...
    method_signature: $ => seq(
        field('name', $.identifier), ':', $.function_signature, optional(',')
    ),

    function_signature: $ => seq(
      field('parameters', $.parameters), '->', field('return_type', $._type), optional($.with_clause)
    ),

    _expression: $ => choice(
      $.identifier, $._literal, $.path_expression, $.parenthesized_expression,
    ...
    path_expression: $ => prec.left(PREC.path, seq(
      field('module', $._expression), '::', field('member', $.identifier)
    )),
    ...
    _pattern: $ => choice(
      $.identifier, $._literal, $.destructuring_pattern
    ),

    destructuring_pattern: $ => prec(PREC.pattern, seq(
      field('type', $.path_expression), '(', field('name', $.identifier), ')'
    )),

    _type: $ => choice(
      $.identifier, $.path_expression, $.generic_type, $.function_type, $.never_type,
      $.handler_type
    ),

    handler_type: $ => prec.right(1, seq(
        'handler', field('effect', $._type), optional($.with_clause)
    )),

    generic_type: $ => seq(field('name', $._type), $.type_parameters),
    type_parameters: $ => seq('<', sep1($._type, ','), '>'),
    function_type: $ => prec.right(1, seq($.parameters, '->', $._type)),
    never_type: $ => '!',
    ...
    with_clause: $ => prec(1, seq(
      'with', choice( $._type, seq('{', sep1($._type, ','), '}'))
    )),
